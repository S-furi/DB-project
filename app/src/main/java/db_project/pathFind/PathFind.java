...
public class PathFind {
  ...
  public PathFind(Graph graph) {
    ...
    for (var i = 0; i < this.numOfVertices; i++) this.parent[i] = -1;
    this.computed = false;
  }

  /**
   * Applying Dijikstra algorithm, will be returned the shortest path for each node in the graph.
   *
   * @param sourceVertex
  ...
  public int[] getMinDistance(int sourceVertex) {
    ...
    for (var i = 0; i < this.numOfVertices; i++) dist[i] = Integer.MAX_VALUE;
