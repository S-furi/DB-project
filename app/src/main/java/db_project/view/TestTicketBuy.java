...
public class TestTicketBuy implements Initializable {
    ...
    private void disableButtons() {
        this.buyTicketButtton.disableProperty()
            .bind(
                this.routeInfoTrainIdChoiceBox.valueProperty().isNull()
                .or(this.routeInfoPathIdChoiceBox.valueProperty().isNull())
                .or(this.routeInfoDatePicker.valueProperty().isNull()));

        this.routeInfoTrainIdChoiceBox.disableProperty()
            .bind(
                this.routeInfoPathIdChoiceBox.valueProperty().isNull());
    }

    private void fillChoiceBoxes() {
        this.routeInfoTrainIdChoiceBox.getItems()
            .setAll(
                this.trainController.getAllTrains()
                    .stream()
                    .map(t -> t.getTrainCode())
                    .collect(Collectors.toList()));
        this.routeInfoPathIdChoiceBox.getItems()
            .setAll(
    ...
    @FXML void registerTicketBought(ActionEvent event) {
