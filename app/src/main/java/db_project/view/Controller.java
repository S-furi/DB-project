...
public class Controller implements Initializable {
  private static final ConnectionProvider connectionProvider =
      ConnectionController.getConnectionProvider();
  private final SampleLoader sample =
      new SampleLoader("data_generation/users/user_data/db_data.json");

  /* Users */
  @FXML private Button eraseButton;
  @FXML private Button registerButton = new Button();
  @FXML private Button columnRemover;
  ...
  @FXML private TextField telField = new TextField();
  @FXML private TableView<User> table = new TableView<>();

  private final UsersController usersController = new UsersController();
  /* Users SQL TABLE */
  private static final UserTable usersTable =
      new UserTable(connectionProvider.getMySQLConnection());

  /* Railway */
  @FXML private TableView<StationWithCheckBox> solutionsTable;
  @FXML private Button saveSolutionsButton;
  @FXML private ChoiceBox<String> dstChoiceBox = new ChoiceBox<>();
  @FXML private ChoiceBox<String> srcChoiceBox = new ChoiceBox<>();
  ...
  public void initialButtonsSetup() {
    ...
    this.searchSolutionsButton
        .disableProperty()
        .bind(this.srcChoiceBox.valueProperty().isNull().or(dstChoiceBox.valueProperty().isNull()));
  ...
  public void createUsersTableView() {
    ...
    this.usersController.getTableViewColumns().forEach(t -> this.table.getColumns().add(t));
  ...
  void eraseAllFields(ActionEvent event) {
    List.of(this.emailField, this.firstnameField, this.lastnameField, this.telField)
        .forEach(t -> t.clear());
  }
  ...
  /**
   * Once the user has selected wich stations to include in the route, this method clears the table
   * and the dropdown boxes and saves the result.
   *
   * @param event pressing confirm button
   */
  @FXML
  void computeRoute(ActionEvent event) {
    final var selectedStations =
        this.solutionsTable.getItems().stream()
