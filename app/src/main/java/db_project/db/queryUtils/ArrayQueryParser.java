...
public class ArrayQueryParser implements QueryParser {
     ...
     * begins with, an according to that, an update is executed (in case of DELETE,
     * UPDATE, INSERT operations) or a result is built
     *
    ...
    private boolean parseAndExecuteQuery() {
        ...
        } else if (this.finalQuery.toString().startsWith("DELETE") || this.finalQuery.toString().startsWith("UPDATE")
     ...
     * hasn't got non-static-final fields inside query's body) or it needs to be a
     * PreparedStatement, in order to insert all the parameters.
     *
     ...
     * got non-static-final parameters and so it's safe (no sql injection) to assign
     * them to the string).
     *
     * <p>
     * For a better understanding about the result go to
     * {@link db_project.db.queryUtils.QueryResult} and read why and what it yields.
     *
     ...
     *         name of the column, and value is the value of the i-th row of the
     *         result.
     */
