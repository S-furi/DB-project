...
public abstract class AbstractCompositeKeyTable<T, K> implements CompositeKeyTable<T, K> {
    ...
    /**
     * Return all the parameters for Update query (all the info about Parameter);
     * In this specific case, the structure of parameters is the following
     * (assuming to have 2 keys and 2 columns):
     * final Object[] params = { key1, key2, col1, col2, key1, key2 };
     * 
     * The syntax is weird, but the query can explain why:
     * UPDATE TABLE_NAME SET key1=?, key2=?, col1=?, col2=? WHERE key1 = ? AND key2
     * = ?
     * 
     * The set on the keys is necessary because the are instances in out DB
     * where a table is formed only by a 3 primary keys, and if you want to change
     * a parameter (e.g. the date forms the PK) it's necessary to specify all the
     * keys (apparently); No better ways was found :) .
     *
     * @param value the value where to retreive the parameters.
     * @return the list of paramentes.
     */
