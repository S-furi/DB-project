...
public class DriverTable extends AbstractTable<Driver, String> {
    ...
    protected Object[] getSaveQueryParameters(final Driver driver) {
        return new Object[] { driver.getLicenceNumber(), Utils.dateToSqlDate(driver.getContractYear()),
                driver.getFirstName(), driver.getLastName(), driver.getTelephone(), driver.getEmail(),
                driver.getResidence() };
    }

    @Override
    protected Object[] getUpdateQueryParameters(final Driver driver) {
        return new Object[] { Utils.dateToSqlDate(driver.getContractYear()), driver.getFirstName(),
                driver.getLastName(), driver.getTelephone(), driver.getEmail(), driver.getResidence(),
    ...
    protected List<Driver> getPrettyResultFromQueryResult(QueryResult result) {
        ...
        result.getResult().get().forEach(row -> {
            ...
            drivers.add(new Driver(licenceNumber, contractYear, firstName, lastName, telephone, email, residence));
        });
        return drivers;
    }
}
