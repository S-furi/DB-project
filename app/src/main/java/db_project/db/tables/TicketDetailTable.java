...
public class TicketDetailTable extends AbstractTable<TicketDetail, String> {
    ...
    public TicketDetailTable(final Connection connection) {
        ...
        super.setTableColumns(List.of("dataPrenotazione", "numClasse", "codTreno", "numeroCarrozza", "numeroPosto"));
    }
    ...
    protected List<TicketDetail> getPrettyResultFromQueryResult(final QueryResult result) {
        ...
        result.getResult().get().forEach(row -> {
            ...
            ticketDetails.add(new TicketDetail(ticketId, reservationDate, trainClass, trainId, carNumber, seatNumber));
        });
